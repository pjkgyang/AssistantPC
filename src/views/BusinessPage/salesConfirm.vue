<template>
  <div class="mg15">
      ...
      <div>
        <div class="mg-12">
          <el-button
            ...
            v-show="!filterData.sfqr"
            @click="handleConfirmOrReject(1)"
          >确认</el-button>
          <el-button
            size="mini"
            type="danger"
          ...
          >
            ...
            <el-table-column prop="zt" label="状态" width="100">
              <template slot-scope="scope">
                <el-tag
                  size="mini"
                  :type="scope.row.zt==3?'success':scope.row.zt==5?'danger':''"
                >{{scope.row.zt==3?'已完成':scope.row.zt==5?'关闭':'完成待确认'}}</el-tag>
              </template>
            </el-table-column>
    ...
    <el-dialog title="打回说明" :visible.sync="dialogVisible" width="500px">
      <div class="mg15">
        <el-input type="textarea" :rows="4" placeholder="请输入说明内容" v-model="sm"></el-input>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      window.open(
        window.baseurl +
          "milestone/exportSaleConfirmMilestone.do?keyword=" +
          this.filterData.keyword +
          "&sfqr=" +
          this.filterData.sfqr
      );
    },
    // 查询确认里程碑 列表
    handleCheck() {
