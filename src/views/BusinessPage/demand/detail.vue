<template>
  <div class="demandDetail-pannel">
      ...
      <div class="demandDetail" colcenter>
        ...
        <div class="demandDetail-info" flex-column spacebetween>
          ...
          <p>
            ...
            <a
              v-if="!!detailInfo.sjbh"
              href="javaScript:;"
              @click="handleClickRwxq(detailInfo.sjbh)"
            >{{detailInfo.sjbh}}</a>
            <span v-if="!detailInfo.sjbh">无</span>
            &#x3000;&#x3000;
            <span class="title">开发编号：</span>
            <a
              v-if="!!detailInfo.kfbh"
              href="javaScript:;"
              @click="handleClickRwxq(detailInfo.kfbh)"
            >{{detailInfo.kfbh}}</a>
          ...
          <p>
            ...
            <a
              title="点击下载"
              :href="API.downloadFile+'?fjId='+detailInfo.xqfjwid"
              target="_blank"
            >{{detailInfo.xqfjmc}}</a>
            &#x3000;&#x3000;
            <span>
              ...
              <a
                v-if="!!detailInfo.xxyxfjmc"
                title="点击下载"
                :href="API.downloadFile+'?fjId='+detailInfo.xxyxfjwid"
                target="_blank"
              >{{detailInfo.xxyxfjmc}}</a>
              <span v-if="!detailInfo.xxyxfjmc">无</span>
            </span>&#x3000;&#x3000;
            <span>
              <span class="title">原型设计：</span>
              <a
                v-if="!!detailInfo.yxfjmc"
                title="点击下载"
                :href="detailInfo.yxfjurl+'&USERID='+userInfo.userName+'&SIGN='+des+'#/fbxq'"
                target="_blank"
              >{{detailInfo.yxfjmc}}</a>
            ...
            <span>
              ...
              <a
                v-if="!!detailInfo.kffjmc"
                title="点击下载"
                :href="detailInfo.kffjurl+'&USERID='+userInfo.userName+'&SIGN='+des+'#/fbxq'"
                target="_blank"
              >{{detailInfo.kffjmc}}</a>
              <span v-if="!detailInfo.kffjmc">无</span>
            </span>
          </p>
        </div>
      </div>
      ...
      <div class="detail-content">
              ...
              <div>
                ...
                <div>
                  ...
                  <p v-if="!!reply.fbfjmc">
                    ...
                    <a
                      title="点击下载"
                      :href="reply.fbwjurl+'&USERID='+userInfo.userName+'&SIGN='+des+'#/fbxq'"
                      target="_blank"
                    >{{reply.fbfjmc}}</a>
                  </p>
                  <p v-if="!!reply.xxyxfjmc">
                    <span>附件：</span>
                    <a
                      title="点击下载"
                      :href="API.downloadFile+'?fjId='+reply.xxyxfjwid"
                      target="_blank"
                    >{{reply.xxyxfjmc}}</a>
                  </p>
                  <p v-if="!!reply.crowdrwbh">
                    <span>任务编号：{{reply.crowdrwbh}} &#x3000;</span>
                    <a href="javaScript:;" @click="handleClickRwxq(reply.crowdrwbh)">查看任务详情</a>
                  </p>
                  <p v-if="!!reply.fjwid">
                    <span>附件：&#x3000;</span>
                    <a
                      title="点击下载"
                      :href="API.downloadFile+'?fjId='+reply.fjwid"
                      target="_blank"
                    >{{reply.fjmc}}</a>
                  </p>
                </div>
        ...
        <div text-right>
          <el-button
            ...
            :key="index"
            @click="handleBtnOprate(btn)"
          >{{btn.btnabb}}</el-button>
        </div>
      </div>
    </div>
    ...
    >
      <div style="padding:20px 10px ">
        <div class="CrowdRwxx">
          <span>任务名称 :</span>
          <span
            :title="JSON.stringify(CrowdRwxx) == '{}' ? '' : CrowdRwxx.rwmc"
            class="Crowdrwmc"
          >{{ JSON.stringify(CrowdRwxx) == '{}' ? '无' : CrowdRwxx.rwmc }}</span>
          <span>交付日期 :</span>
    ...
    <xqqrDialog
      ...
      :zbwid="zbwid"
      :btnbh="btnInfo.btnid"
      :demandDetail="detailInfo"
      @handleClickSure="handleCommitQrxq"
    ></xqqrDialog>
    <!-- 需求确认 -->
    <yzDialog
      :show.sync="yzShow"
      :zbwid="zbwid"
      :btnbh="btnInfo.btnid"
      :xmbh="detailInfo.xmbh"
      @handleClickSure="handleClickYzxq"
    ></yzDialog>
    <!-- 需求验证 -->
    <gbxqDialog
      :show.sync="gbxqShow"
      ...
      :btnbh="btnInfo.btnid"
      :demandDetail="detailInfo"
      @handleClickSure="handleClickGbxq"
    ></gbxqDialog>
    <!-- 关闭需求 -->
    <crowdDialog
      :show.sync="crowdShow"
      ...
      :zbwid="zbwid"
      :btnbh="btnInfo.btnid"
      @handleCommitCrowd="handleCommitCrowd"
    ></crowdDialog>
    <!--  crowd -->
    <bhDialog :show.sync="bhShow" @handleClickSure="handleRejct"></bhDialog>
    <!-- 驳回 -->
    <xqtbDialog
      :show.sync="xqtbShow"
      :zbwid="zbwid"
      :btnbh="btnInfo.btnid"
      :xmbh="detailInfo.xmbh"
      @handleClickSure="handleClickXqtb"
    ></xqtbDialog>
    <!-- 驳回  -->
    <fpgcsDialog :show.sync="fpgcsShow" @handleCommitKfgcs="handleCommitKfgcs"></fpgcsDialog>
    <!-- 分配工程师  -->
    <!-- 发布需求，编辑 -->
    <fbxqDialog
      :show.sync="fbxqShow"
      :demandInfo="detailInfo"
      :Type="xqType"
      :btnbh="btnInfo.btnid"
      @handleCommitDemand="handleCommitDemand"
    ></fbxqDialog>
    <!-- 提交开发包  -->
    <kfbDialog
      :show.sync="kfbShow"
...
import { encryptByDES,formatTime } from "@/utils/util.js";
...
export default {
  ...
  methods: {
    ...
    handleBtnOprate(params) {
      this.btnInfo = params;
      if (params.btnid == "1" || params.btnid == "4" || params.btnid == "2") {
        //1回复 4催办 2受理
        if (
          $("#summernoteDemand").summernote("code") == "<p><br></p>" &&
          params.btnid == "1"
        ) {
        ...
        //驳回
        this.bhShow = true;
      } else if (params.btnid == "5") {
        //分配开发工程
        this.fpgcsShow = true;
      } else if (params.btnid == "6") {
        //绿色通道
        this.curType = "lstd";
        this.crowdShow = true;
      } else if (params.btnid == "7") {
        //需求提报
        this.xqtbShow = true;
      } else if (params.btnid == "8") {
        //需求确认
        this.xqqrShow = true;
      } else if (params.btnid == "9") {
        //需求开发
        this.curType = "";
        this.crowdShow = true;
      } else if (params.btnid == "10") {
        //发布需求
        this.xqType = "disabled";
        this.fbxqShow = true;
      } else if (params.btnid == "11") {
        //完成验证
        this.yzShow = true;
      } else if (params.btnid == "12") {
        //关闭需求
        this.gbxqShow = true;
      } else if (params.btnid == "15") {
        //编辑
