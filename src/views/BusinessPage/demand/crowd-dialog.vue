<template>
  <div>
    <el-dialog
      :title="Type == 'lstd'?'绿色通道':'提报crowd任务'"
      width="1000px"
      top="30px"
      ...
      <div class="pd-10">
        <el-form :model="crowdxqData" :rules="rules" ref="crowdxqData" class="demo-ruleForm">
          ...
          <el-form-item>
            <el-select
              size="mini"
              v-model="crowdxqData.sfjj"
              placeholder="请选择需求紧急类型"
              style="width: 100%;"
            >
          ...
          <el-form-item>
            <el-select
              size="mini"
              v-model="crowdxqData.sfxyzc"
              placeholder="请选择是否需要驻场"
              style="width: 100%;"
            >
          ...
          <el-form-item>
            <el-input
              size="mini"
              v-model="crowdxqData.xmysje"
              :readonly="crowdxqData.rwlx == '2'"
          ...
          <el-form-item>
            <el-input
              type="textarea"
              placeholder="请输入机密信息 , 可填写项目的账号、密码等信息,只对中标用户显示"
              v-model="crowdxqData.jmxx"
            ></el-input>
          </el-form-item>
          <!-- 需求描述 -->
...
import { getMenu, getSession, GetDateStr } from "@/utils/util.js";
    ...
    sendSubmitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$post(
            !this.Type
              ? this.API.projectTeamSubmitDvpt
              : this.API.openGreenChannel,
            this.crowdxqData
          ).then(res => {
