      ...
      <div class="pannelPaddingBg-10">
        <div>
          ...
          <el-input
            ...
            style="width:493px;"
            size="mini"
            placeholder="请输入提出人姓名/工号/手机号/标题/项目编号/项目名称"
            @change="handleQuery"
          ></el-input>&#x3000;
        ...
        <div v-show="queryLJshow">
          <div class="mg-12" flex>
            ...
            <p class="query-list">
              ...
              <span
                v-for="(xqfl, index) in xqflList"
                @click="handleChangeXqfl(xqfl.label)"
                :key="index"
                :class="{ 'bg-active': filterData.xqfl == xqfl.label }"
              >{{ xqfl.mc }}</span>
            </p>
          </div>
          <div flex>
            ...
            <p class="query-list">
              ...
              <span
                v-for="(xqzt, index) in xqztList"
                @click="handleChangexqzt(xqzt.label)"
                :key="index"
                :class="{ 'bg-active': filterData.xqzt == xqzt.label }"
              >{{ xqzt.mc }}</span>
            </p>
          </div>
          <div class="mg-12" flex>
            ...
            <p class="query-list">
              ...
              <span
                v-for="(xqlx, index) in xqlxList"
                @click="handleChangeXqlx(xqlx.label)"
                :key="index"
                :class="{ 'bg-active': filterData.xqlx == xqlx.label }"
              >{{ xqlx.mc }}</span>
            </p>
          </div>
          <div flex>
          ...
          <div flex class="mg-12" v-if="groupTag.includes('JZGCRY')||groupTag.includes('JYGL')">
            ...
            <p class="query-list" style="width:90%;">
              ...
              <span
                v-for="(gczd, index) in gczdList"
                @click="handleChangeGczd(gczd.label)"
                :key="index"
                :class="{ 'bg-active': filterData.gczd == gczd.label }"
              >{{ gczd.mc }}</span>
            </p>
          </div>
        </div>
        ...
        <ul class="demand-list">
          <li flex v-for="(item, index) in dataList" :key="index">
            ...
            <div class="demand-list-info" flex-column spacebetween>
              <span>
                <a :href="'#/demand/detail?id='+item.wid" target="_blank">{{item.bt}}</a>
              </span>
              <p>
