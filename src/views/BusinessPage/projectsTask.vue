            ...
            <div>
              <li v-if="taskList.length != 0 && taskList != null" :class="{'list-active':shownindex === index}" v-for="(task,index) of taskList" :key="index" @click="getTaskinfo(task,index)">
                <p>
                  ...
                  <span v-if="Operatepower || tagGroup.includes('JYGL') || tagGroup.includes('QYZ')" @click.prevent="changeZrr($event,index)" title="编辑人员" style="float:right;margin-top:3px;">
          ...
          <transition name="el-zoom-in-top">
            <div v-if="radio == 'bgst' && ViewTransition" style="background:#fff;margin:0 10px;border-radius:5px">
              ...
              <div text-right style="margin:0 11px;padding:5px 0">
                <el-pagination @size-change="handleSizeChangeBg" @current-change="handleCurrentChangeBg" :current-page="bgCurrentPage" :page-sizes="[20, 35, 50 , 100]" :page-size="bgPageSize" layout="total, sizes, prev, pager, next, jumper" :total="bgTotal">
                </el-pagination>
              </div>
            </div>
          </transition>
          ...
          <div>
            ...
            <div class="detail-activities-wrap">
              <div class="involve-view">
                <p>参与者</p>
                <el-tag :key="index" v-for="(tag,index) in dynamicTags" :closable="tag.del" :disable-transitions="false" @close="handleClose(tag)">
                  {{tag.userName}}
                </el-tag>
                ...
                <ul>
                  <li v-if="taskDetailProcess.length != 0 && taskDetailProcess != null" class="task-detail-process" v-for="(process,index) in taskDetailProcess">
                    <div v-if="process.gclx == 5" style="padding:3px 0;">
                      <div>
                        <p>
                          <span :title="process.gcms" :class="{'el-icon-plus':process.czlx==1,'el-icon-delete':process.czlx==2,'el-icon-refresh':process.czlx==3}" class="process-daily">
                            {{process.gcms}}</span>
          ...
          <div class="activity-creator">
            <div class="activity-creator-content">
              ...
              <div class="activity-creator-operate">
                <el-upload class="upload-demo" ref="upload" action="dis" multiple :before-upload="beforeUpload" :on-remove="handleRemove" :on-change="handleChange" :file-list="fileList" :auto-upload="false">
...
export default {
  data() {
    return {
      ...
      value: "",
      taskSSry: [],
      options: [{ label: "未完成", value: 1 }, { label: "已完成", value: 2 }],
      optionsSort: [
