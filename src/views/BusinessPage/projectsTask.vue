<template>
  <div style="height:100%;overflow-x:auto">
    <div class="myItem-top-bread">
      <user-banner
        :shown="false"
        :Operatepower="Operatepower"
        :bannerACtive="tabsLabel"
        :itemMc="'['+xmkbInfo.xmbh+']'+xmkbInfo.xmmc"
        :fbdetail="true"
        :userNum="itemUserNum"
        @handleAddDDgroup="handleAddDDgroup"
        @handleItemUser="userListShow = !userListShow"
        :xmbh="xmbh"
        :xmData="xmkbInfo"
      ...
      <div v-if="tabsLabel == 'process'" style="display:flex">
        <div class="project_task_detail_taskbar">
          <div class="project_info">
            ...
            <div style="margin-bottom:5px;">
              ...
              <el-select
                ...
                placeholder="请选择任务状态"
                size="mini"
                @change="handleChangeRwzt"
              >
                <el-option
                  v-for="item in rwztOptions"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
          ...
          <ul class="task_list">
            <div>
              <li
                v-if="taskList.length != 0 && taskList != null"
                :class="{'list-active':shownindex === index}"
                v-for="(task,index) of taskList"
                :key="index"
                @click="getTaskinfo(task,index)"
              >
                <p>
                  <strong
                    :title="task.cp_display"
                    :class="{'on_active':shownindex === index,'project_task_detail_cpmc':true}"
                  >{{task.cp_display}}</strong>
                  <br />
                  ...
                  <span
                    v-if="Operatepower || tagGroup.includes('JYGL') || tagGroup.includes('QYZ')"
                    @click.prevent="changeZrr($event,index)"
                    title="编辑人员"
                ...
                <div style="margin:5px 0;">
                  <el-progress
                    :show-text="false"
                    :percentage="task.rwzs == 0?0:task.ywcrwzs == 0?0:(task.ywcrwzs/task.rwzs)*100"
                    status="success"
                  ></el-progress>
                </div>
              </li>
          ...
          <ul class="task_list task_complete_status">
            <li
              @click="checkMyTask"
              data-type="Mytask"
              :class="{'task-active':taskActive == 'Mytask'}"
            >
              <span class="el-icon-news"></span> 我的任务
            </li>
          </ul>
        </div>
        ...
        <div class="project_task_milestone">
          <div style="height:100%;" class="milestone-list" v-if="ViewTransition && radio != 'bgst'">
            ...
            >
              <itemTask
                ...
                :cpData="cpData"
                :rwzt="rwztValue"
                :TaskDatas="milestone.tasks.rows"
                :index="index"
                :milestone="milestone"
            ...
            <div
              v-if="taskList.length == 0 || taskList == null"
              style="text-align:center;padding-top:150px;width:100%"
            >
          ...
          <div
            v-if="!ViewTransition"
            class="my-table"
            style="padding:0 10px;min-width:1040px;background:#fff;margin:0 10px;border-radius:5px"
          >
            <div
              style="display:flex;justify-content:space-between;border-bottom:2px solid #e4e7ed;height:40px;margin:0 0 20px 10px;"
            >
            ...
            <itemtable
              :tableData="myTaskArr"
              @handleDailyParper="handleDailyParper"
          ...
          <transition name="el-zoom-in-top">
            <div
              v-if="radio == 'bgst' && ViewTransition"
              style="background:#fff;margin:0 10px;border-radius:5px"
            >
              <div
                flex
                spacebetween
                style="border-bottom:2px solid rgb(228, 231, 237);margin:0 10px 5px;padding:5px 0"
              >
                <div>
                  <el-checkbox
                    v-model="sfxgValue"
                  ...
                  ></el-checkbox>&#x3000;
                  <el-button
                    v-if="rwztValue == '1' && radio == 'bgst' "
                    type="danger"
                    size="mini"
                    @click="handleCloseBatchlot"
                  >批量关闭</el-button>
                  <el-button
                    v-if="rwztValue == '2' && radio == 'bgst'"
                    type="primary"
                    size="mini"
              ...
              <taskTable
                :tableData="bgstList"
                :rwzt="rwztValue"
                @handlejfqr="handleDialog"
                @handletask="handleDialog"
            ...
            <div class="task-detail-scenario-infos-wrap">
              ...
              <p>
                <span
                  ...
                  v-if="lcbTasks.lx != 9"
                >{{lcbTasks.lx==1?' 里程碑模板':' 模板'}}</span>
                <a
                  v-if="lcbTasks.lx != 9 && TemplateFile.mblj.split('.')[1] != undefined"
                  class="LcbTemplate"
                  v-for="(TemplateFile,index) in lcbTemplateFile"
                  :key="index"
                  :href="baseUrl+'attachment/downloadTemplateFile.do?path='+TemplateFile.mblj"
                  target="blank"
                >{{TemplateFile.wdmc+'.'+TemplateFile.mblj.split('.')[TemplateFile.mblj.split('.').length-1]}}</a>
                <span
                  v-if="lcbTasks.lx != 9 &&(lcbTemplateFile.length == 0 || lcbTemplateFile == null)"
                >暂无</span>
              </p>
            </div>
            ...
            <div class="detail-activities-wrap">
              <div class="involve-view">
                ...
                <el-button
                  ...
                  type="primary"
                  @click="addUser"
                  v-if="tasksInfo.lx != 1 && Operatepower"
                >+</el-button>
                <transition name="fade">
                  <ul class="task_participant" v-if="selectShow">
                    ...
                    <ul style="height:180px;overflow-y:auto">
                      <li
                        v-for="(list,index) in listt0"
                        :data-label="list.userName+'&'+list.userId+'&'+index"
                        :key="index"
                        @click="addTaskParty"
                      >
              ...
              <div class="activities-list-wrap">
                <ul>
                  <li
                    v-if="taskDetailProcess.length != 0 && taskDetailProcess != null"
                    class="task-detail-process"
                    ...
                    <div v-if="process.gclx == 5" style="padding:3px 0;">
                      <div>
                        <p>
                          <span
                            :title="process.gcms"
                            :class="{'el-icon-plus':process.czlx==1,'el-icon-delete':process.czlx==2,'el-icon-refresh':process.czlx==3}"
                            class="process-daily"
                          >{{process.gcms}}</span>
                      ...
                      <div>
                        <p style="line-height:32px;">
                          <span style="float:left;height:32px;">
                            <img src="static/img/defaultAvatar.png" :onerror="errorImg" />&nbsp;
                          </span>
                          <span style="float:left;color:#999;">
                            {{process.cjrxm}} &#x3000;{{process.userCode}}
                            <span
                              v-if="process.gclx == 2 ||process.gclx == 3"
                              :class="{'process-gclx':true,'gclx-rz':process.gclx == 2,'gclx-cb':process.gclx == 3}"
                            >{{process.gclx == 2?'日报':'催办'}}</span>
                          </span>
                          <span style="float:right;color:#aaa">{{process.cjsj}}</span>
                        </p>
                        ...
                        <p>
                          <a
                            class="process-fjlist"
                            v-for="(fj,index) in process.fjData"
                            :key="index"
                            :href="baseUrl + 'attachment/downloadFile.do?fjId='+fj.fjbh"
                            target="blank"
                          >{{fj.fjmc}}</a>
                        </p>
                      </div>
                ...
                <div
                  class="taskProcess-tips"
                  v-if="taskDetailProcess.length != 0 && taskDetailProcess != null"
                >
                  <a v-if="(taskProcessNum - curPageNum) > 0" @click="showAllprocess">
                    显示较早的 {{taskProcessNum - curPageNum}}
                    条动态
                  </a>
                </div>
              </div>
        ...
        >
          <enditTaskInfo
            @changeCPzrr="changeCPzrr"
            :xmbh="xmbh"
            :cpinfo="cpinfo"
            @handleCommit="handleEditTaskCommit"
            :dialogEditTaskVisible="dialogEditTaskVisible"
          ></enditTaskInfo>
        </el-dialog>
        <!-- 编辑任务 -->
        <el-dialog
          title="编辑任务"
          @close="closeDialog"
          :visible.sync="dialogEditLcbTaskVisible"
          :close-on-click-modal="false"
        >
          <editLcbTasks
            :dialog="dialog"
            @handleEditTaskSuc="handleEditTaskSuc"
            :lcbrw="LcbrwArr"
            :taskData="editTaskInfo"
          ></editLcbTasks>
        </el-dialog>
        <!-- 提报里程碑 -->
        <el-dialog
          title="提报里程碑"
          :visible.sync="commitlcbVisible"
          top="50px"
          width="600px"
          :close-on-click-modal="false"
        >
          <commitMilestone
            :shown="commitlcbVisible"
            :taskDetail="lcbTaskDetail"
      ...
      <el-collapse-transition>
        <div v-if="tabsLabel == 'overview'">
          <overview
            ...
            :xmbh="xmbh"
            :label="tabsLabel"
            @checkSkipDetail="checkSkipDetail"
          ></overview>
        </div>
      </el-collapse-transition>
      ...
      <el-collapse-transition>
        <div v-if="tabsLabel == 'complain'">
          <complain
            ...
            :xmmc="xmkbInfo.xmmc"
            :xmkbInfo="xmkbInfo"
            @handleComplanit="handleComplanit"
          ></complain>
        </div>
      </el-collapse-transition>
    ...
    <el-dialog
      title="钉钉群"
      :visible.sync="dingdingGroupVisible"
      width="400px"
      :close-on-click-modal="false"
    >
...
export default {
  ...
  activated() {
    ...
    if (
      this.tagGroup.includes("JZGCRY") &&
      (!this.tagGroup.includes("ZDDZ") ||
        !this.tagGroup.includes("JYGL") ||
        !this.tagGroup.includes("GCZJ") ||
        !this.tagGroup.includes("QYZ")) &&
      window.userName != this.xmkbInfo.yfzrrxm
    ) {
      this.isMine = 1;
    } else {
      this.isMine = 0;
