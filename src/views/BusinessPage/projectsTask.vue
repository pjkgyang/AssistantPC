<template>
  <div style="height:100%;overflow-x:auto">
    <div class="myItem-top-bread">
      <user-banner :shown="false" :Operatepower="Operatepower" :bannerACtive="tabsLabel" :itemMc="'['+xmkbInfo.xmbh+']'+xmkbInfo.xmmc"
        :fbdetail="true" :userNum="itemUserNum" @handleAddDDgroup="handleAddDDgroup" @handleItemUser="userListShow = !userListShow"
        :xmbh="xmbh" :xmData="xmkbInfo" @handleTabsClick="handleTabsClick"></user-banner>
    ...
    <div class="project_task_detail" @click.stop="selectHide">
      <div v-if="tabsLabel == 'process'" style="display:flex">
        <div class="project_task_detail_taskbar">
          ...
          <ul class="task_list">
            <div>
              <li v-if="taskList.length != 0 && taskList != null" :class="{'list-active':shownindex === index}" v-for="(task,index) of taskList"
                :key="index" @click="getTaskinfo(task,index)">
                <p>
                  ...
                  <span>{{task.jfzrrxm == ''?'暂无':task.jfzrrxm}}</span>&nbsp;&nbsp;
                  <span class="item_zrr item_zrr_YF">乙</span>
                  <span>{{task.yfzrrxm}}</span>

                  <span v-if="Operatepower || tagGroup.includes('JYGL') || tagGroup.includes('QYZ')" @click.prevent="changeZrr($event,index)"
                    title="编辑人员" style="float:right;margin-top:3px;">
                    <img src="static/img/usergl.png" alt="">
                  </span>
                </p>
                <div style="margin:5px 0;">
                  <el-progress :show-text='false' :percentage="task.rwzs == 0?0:task.ywcrwzs == 0?0:(task.ywcrwzs/task.rwzs)*100"
          ...
          <ul class="task_list task_complete_status">
            <li @click="checkMyTask" data-type="Mytask" :class="{'task-active':taskActive == 'Mytask'}">
              <span class="el-icon-news"></span> 我的任务</li>
          </ul>
        </div>
        ...
        <div class="project_task_milestone">
          <div style="height:100%;" class="milestone-list" v-if="ViewTransition && radio != 'bgst'">
            <div v-for="(milestone,index) in milestones" :key="index" v-if="taskList.length != 0 && taskList != null">
              <itemTask :rwlx="rwlxValue" :cpData="cpData" :rwzt="rwztValue" :TaskDatas="milestone.tasks.rows" :index="index"
                :milestone="milestone" @handleDialog="handleDialog" @handleTaskinfo="handleTaskinfo"></itemTask>
            </div>
          ...
          <div v-if="!ViewTransition" class="my-table" style="padding:0 10px;min-width:1040px;background:#fff;margin:0 10px;border-radius:5px">
            ...
            <itemtable :tableData="myTaskArr" @handleDailyParper="handleDailyParper" @changeTaskStaus="changeTaskStaus"
              @handleCell="handleCell"></itemtable>
            <div style="text-align:right;margin-top:20px;">
              <!-- 分页 -->
              <pagination v-if="records != 0 && records != null" :pageSize="pageSize" :total="records"
          ...
          <transition name="el-zoom-in-top">
            <div v-if="radio == 'bgst' && ViewTransition" style="background:#fff;margin:0 10px;border-radius:5px">
              ...
              <div text-right style="margin:0 11px;padding:5px 0">
                <el-pagination @size-change="handleSizeChangeBg" @current-change="handleCurrentChangeBg" :current-page="bgCurrentPage"
                  :page-sizes="[20, 35, 50 , 100]" :page-size="bgPageSize" layout="total, sizes, prev, pager, next, jumper"
                  :total="bgTotal">
                </el-pagination>
              </div>
            </div>
          ...
          <div>
            ...
            <div class="task-detail-scenario-infos-wrap">
              <p>
                ...
                <span v-if="lcbTasks.lx != 1 && Operatepower">
                  <span v-if="!xgssry">
                    <el-select v-model="value" size='mini' filterable :filter-method="filterSearch" @change="getTaskssry"
                      placeholder="请选择/搜索用户姓名/编号">
                      <el-option v-for="(item,index) in taskSSry" :key="index" :label="item.userName" :value="item.userName+'&'+item.userId">
                      </el-option>
                    </el-select>
                  </span>
              ...
              <p>
                <span style="display:inline-block;width:105px" class="el-icon-news"> 任务状态</span>
                <span>{{lcbTasks.zt_display}}</span>
              </p>
              <p>
                <span style="display:inline-block;width:105px" class="el-icon-sort"> 优先级</span>
                <span>{{lcbTasks.yxj_display}}</span>
              </p>
              <p>
                <span style="display:inline-block;width:105px" class="el-icon-time"> 工时</span>
                <span>{{lcbTasks.jhgs}}&#x3000;</span>
              </p>
              <p>
                <span style="display:inline-block;width:105px" class="el-icon-date"> 实际结束日期</span>
                <span>{{!lcbTasks.sjjsrq?'无':lcbTasks.sjjsrq}}&#x3000;</span>
              </p>
              <p>
                <span style="display:inline-block;width:105px" class="el-icon-document"> 任务内容</span>
                <span>{{lcbTasks.rwxq==''?'无':lcbTasks.rwxq}}</span>
              </p>
              <p>
                <span style="display:inline-block;width:105px" class="el-icon-document" v-if="lcbTasks.lx != 9">{{lcbTasks.lx==1?' 里程碑模板':' 模板'}}</span>
                <a v-if="lcbTasks.lx != 9 && TemplateFile.mblj.split('.')[1] != undefined" class="LcbTemplate" v-for="(TemplateFile,index) in lcbTemplateFile"
                  :key="index" :href="baseUrl+'attachment/downloadTemplateFile.do?path='+TemplateFile.mblj" target="blank">
            ...
            <div class="detail-activities-wrap">
              <div class="involve-view">
                <p>参与者</p>
                <el-tag :key="index" v-for="(tag,index) in dynamicTags" :closable="tag.del" :disable-transitions="false"
                ...
                <transition name="fade">
                  <ul class="task_participant" v-if="selectShow">
                    <li>
                      <el-input v-model="name" placeholder="搜索姓名" @focus="searchPerson"></el-input>
                    </li>
                    <ul style="height:180px;overflow-y:auto">
                      <li v-for="(list,index) in listt0" :data-label="list.userName+'&'+list.userId+'&'+index" :key="index"
                        @click="addTaskParty">
              ...
              <div class="activities-list-wrap">
                <ul>
                  <li v-if="taskDetailProcess.length != 0 && taskDetailProcess != null" class="task-detail-process"
                    v-for="(process,index) in taskDetailProcess">
                    <div v-if="process.gclx == 5" style="padding:3px 0;">
                      <div>
                        <p>
                          <span :title="process.gcms" :class="{'el-icon-plus':process.czlx==1,'el-icon-delete':process.czlx==2,'el-icon-refresh':process.czlx==3}"
                            class="process-daily">
                            {{process.gcms}}</span>
                    ...
                    <div v-if="process.gclx != 5">
                      <div>
                        ...
                        <p>
                          <a class="process-fjlist" v-for="(fj,index) in process.fjData" :key="index" :href="baseUrl + 'attachment/downloadFile.do?fjId='+fj.fjbh"
                            target="blank">{{fj.fjmc}}</a>
                        </p>
                      </div>
                    </div>
                  </li>
                ...
                <div class="taskProcess-tips" v-if="taskDetailProcess.length != 0 && taskDetailProcess != null">
                  <a v-if="(taskProcessNum - curPageNum) > 0" @click="showAllprocess">显示较早的 {{taskProcessNum - curPageNum}}
                    条动态</a>
                </div>
              </div>
            </div>
          ...
          <div class="activity-creator">
            <div class="activity-creator-content">
              ...
              <div class="activity-creator-operate">
                <el-upload class="upload-demo" ref="upload" action="dis" multiple :before-upload="beforeUpload"
        ...
        <el-dialog title="填写日报" :visible.sync="dialogDailyVisible" @close="closeDailyParper" :close-on-click-modal="false">
          <dailyParper @handleSubmit="handleSubmit" :taskName="taskName" :xmbh="xmbh" :logInfo="TaskProcess"
            :closeDialogNum="closeDialogNum"></dailyParper>
        </el-dialog>
        <!-- 编辑产品 -->
        <el-dialog title="修改产品信息" :visible.sync="dialogEditTaskVisible" :close-on-click-modal="false">
          <enditTaskInfo @changeCPzrr="changeCPzrr" :xmbh="xmbh" :cpinfo="cpinfo" @handleCommit="handleEditTaskCommit"
            :dialogEditTaskVisible="dialogEditTaskVisible"></enditTaskInfo>
        </el-dialog>
        <!-- 编辑任务 -->
  ...
  export default {
    ...
    methods: {
      ...
      handleTabsClick(data) {
        ...
        if (data.name == "process") {
          ...
          if (!!localStorage.getItem("jdst")) {
            this.radio = localStorage.getItem("jdst");
          } else {
            this.radio = "kbst";
          }
        }
        ...
        queryTaskProcess({
          ...
          if (data.state == "success") {
            if (data.data.rows != null) {
              this.taskDetailProcess = this.taskDetailProcess.concat(
                data.data.rows
              );
      ...
      handleDailyParper(data) {
        ...
        this.closeDialogNum = this.closeDialogNum - 1;
        this.taskName =
          data.xmmc + " — " + data.cpmc_display + " — " + data.rwmc_display;
        this.dialogDailyVisible = !this.dialogDailyVisible;
      },
      // 提交里程碑（表格）
      ...
      CommitChangeTaskSSry() {
        ...
        if (JSON.stringify(this.xztaskSSry) === "{}") {
          ...
          this.xztaskSSry.userId == "" ||
          this.xztaskSSry.userName == ""
        ) {
          ...
          if (data.state == "success") {
            ...
            this.$alert("修改成功", "提示", {
              ...
              callback: action => {
                ...
                this.milestones[this.tasksInfo.lcbindex].tasks.rows[
                  index
                ].ssrxm = this.xztaskSSry.userName;
              }
            });
          }
        });
      ...
      dialogTableVisible(n, o) {
        if (n) {
          this.$nextTick(function() {
            this.$refs.fileContent.appendChild(
              this.$refs.upload.$children[1].$el
            );
          });
