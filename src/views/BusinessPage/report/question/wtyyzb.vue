...
export default {
  ...
  methods: {
    exportTable() {
      let ksrq = !this.filterData.date[0]?'':this.filterData.date[0];
      let jsrq = !this.filterData.date[1]?'':this.filterData.date[1];
      window.open(this.API.exportOperatIndicator+'?qygc='+this.filterData.gczd+'&dwlx='+this.filterData.dwlx+'&startDt='+ksrq+'&endDt='+jsrq);
    },
