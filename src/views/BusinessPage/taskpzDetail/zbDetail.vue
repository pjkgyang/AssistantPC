    ...
    <div>
      <tableLayout>
        ...
        <section slot="bottom">
          <el-table :data="tableDataLcb" border style="width: 100%">
            <el-table-column label="批注" width="120">
              <template slot-scope="scope">
                <a
                  href="javaScript:;;"
                  @click="handleCheckpzDetail(scope.row.zgzWid,3)"
                >{{scope.row.pzzs}}人批注</a>
              </template>
            </el-table-column>
          ...
          <div text-right>
            <el-pagination
              @size-change="handleSizeChangeLcb"
              @current-change="handleCurrentChangeLcb"
              :current-page="currentPageLcb"
              :page-sizes="[15, 25, 35, 50]"
              :page-size="pageSizeLcb"
          ...
          <el-table :data="tableDataJd" border style="width: 100%">
            <el-table-column label="批注" width="120">
              <template slot-scope="scope">
                <a
                  href="javaScript:;;"
                  @click="handleCheckpzDetail(scope.row.wid,5)"
                >{{scope.row.pzzs}}人批注</a>
              </template>
            </el-table-column>
            <el-table-column prop="xmbh" label="项目编号" width="150"></el-table-column>
            ...
            <el-table-column label="添加阶段" width="120">
              <template slot-scope="scope">
                <el-tag
                  size="mini"
                  :type="!scope.row.tjjd?'danger':''"
                >{{scope.row.tjjd == 1?'计划中':scope.row.tjjd == 2?'总结时':scope.row.tjjd == 3?'增补':'未知'}}</el-tag>
              </template>
            </el-table-column>
            <el-table-column prop="zrrxm" label="责任人" width="100"></el-table-column>
          </el-table>
      ...
      <tableLayout>
        ...
        <section slot="bottom">
          <el-table :data="tableDataWt" border style="width: 100%">
            <el-table-column label="批注" width="120">
              <template slot-scope="scope">
                <a
                  href="javaScript:;;"
                  @click="handleCheckpzDetail(scope.row.zwtWid,4)"
                >{{scope.row.pzzs}}人批注</a>
              </template>
            </el-table-column>
            <el-table-column
          ...
          <div text-right>
            <el-pagination
              @size-change="handleSizeChangeWt"
              @current-change="handleCurrentChangeWt"
              :current-page="currentPageWt"
              :page-sizes="[100, 200, 300, 400]"
              :page-size="pageSizeWt"
              layout="total, sizes, prev, pager, next, jumper"
              :total="totalWt"
            ></el-pagination>
          </div>
