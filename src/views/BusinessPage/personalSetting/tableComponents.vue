    ...
    <div>
        ...
        <div class="out-table">
            <el-table :data="bodyData" border class="report-ele-table" width="100%" :max-height="Height==0?'auto':tableHeight">
                ...
                :min-width="widthArr.includes(index)?rowWidth:index==0?200:Width" show-overflow-tooltip :fixed="indexArr.includes(index)?true:false">
                    <template slot-scope="scope">
                        <span v-if="!th.canRedirect">{{scope.row[index]}}</span>
                        <a v-else href="javaScript:void(0)" @click="handleRouter(scope.row,index,'sqjs')">{{scope.row[index]==""?'结算申请':scope.row[index]}}</a>
                        <a v-if="scope.row[index] == ''" href="javaScript:void(0)" @click="handleRouter(scope.row,index,'jscx')">&#x3000;&#x3000;结算申请查询</a>
                    </template>
                </el-table-column>
            </el-table>
        </div>
