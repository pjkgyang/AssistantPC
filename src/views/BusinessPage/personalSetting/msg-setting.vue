  ...
  <div>
    <div class="personal-setting-xxsz">
      <div>
        ...
        <el-checkbox
          :title="sort.text"
          v-for="(sort,i) in checkboxGroup"
          :key="i"
          :checked="sort.value"
          :label="sort.text"
          size="small"
          border
          @change="changeMessageSort(sort)"
        ></el-checkbox>
      </div>
      <br />
...
export default {
  ...
  mounted() {
    ...
    getMessageTypeSwitch().then(({ data }) => {
      //获取消息类型设置
      if (data.state == "success") {
