      ...
      <li v-for="(file,index) in fileList.files" style="padding:0 20px;display:flex" :data-fj="file.fjmc+'&'+file.sfwjml" :data-fjbh="file.fjbh" @click="handleFile">
        <div style="width:40%;padding:6px 0">
          ...
          <div v-if="file.sfwjml == 0">
            <a :href="baseUrl+'attachment/downloadNewcomerFile.do?path='+file.fjbh" target="blank">
...
export default {
  ...
  mounted() {
    ...
    this.openNewcomerFolder();
  },

  methods: {
    openNewcomerFolder(path) {
      this.$get(this.API.openNewcomerFolder, {
        path: path
      }).then(res => {
