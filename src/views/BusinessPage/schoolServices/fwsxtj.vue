			...
			<div>
				...
				<div flex class="mg-12">
					...
					<p class="query-list">
						...
						<span v-for="(fwlb,index) in fwlbList" :class="{ 'bg-active': fwlb.id == filterData.fwlb }" :key="index" @click="CheckFwlb(fwlb.id)">{{ fwlb.text }}</span>
					</p>
				</div>
				...
				<div flex class="mg-12">
					...
					<p class="query-list">
						<span v-for="(fwzn,index) in fwznList" :class="{ 'bg-active': fwzn.id == filterData.fwzn }" :key="index" @click="CheckFwzn(fwzn.id)">{{ fwzn.label }}</span>
					</p>
				</div>
				<div flex class="mg-12">
					<span class="query-title">信息化支持:</span>
					<p class="query-list">
						<span v-for="(xxhzc,index) in xxhzcList" :class="{ 'bg-active': xxhzc.id == filterData.xxhzc }" :key="index" @click="CheckXxhzc(xxhzc.id)">{{ xxhzc.label }}</span>
					</p>
				</div>
				<div flex class="mg-12">
					<span class="query-title">在建状态:</span>
					<p class="query-list">
						<span v-for="(jszt,index) in jsztList" :class="{ 'bg-active': jszt.id == filterData.jszt }" :key="index" @click="CheckZt(jszt.id)">{{ jszt.label }}</span>
					</p>
				</div>
			</div>
			<div class="pd-10">
...
export default {
	data() {
		return {
			fwdxList: [], //服务对象
			fwlbList: [], //服务类别
			fwznList: [{id:'',label:'全部'},{ id: '1', label: '有' },{id:'0',label:'无'}], //服务指南
			xxhzcList: [{ id: '', label: '全部' }, { id: '1', label: '有' }, { id: '0', label: '无' }],
			jsztList: [{ id: '', label: '全部' }, { id: '1', label: '在建' }, { id: '2', label: '售后' }, { id: '2', label: '服务' }],
			filterData: {
				fwlb: '',
				fwdx: [],
				fwzn:'',
