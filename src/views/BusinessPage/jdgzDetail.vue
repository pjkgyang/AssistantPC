<template>
  <div class="pd-10">
    <div class="dialog-jd">
      ...
      <h4
        v-else
      >{{this.$route.query.personType == 1?'实施 — ':'用户 — '}} {{this.$route.query.filterType==1?'催办': this.$route.query.filterType==2?'已超期': this.$route.query.filterType==7?'待确认任务数': this.$route.query.filterType==8?'已确认任务数': this.$route.query.filterType==0?'总数': this.$route.query.filterType==5?'个人任务总数':'未完成个人任务总数'}}</h4>
      <el-table
        :data="tableData"
        border
        style="width: 100%"
      ...
      <div v-if="this.$route.query.lcbqr == '1'">
        <div text-right class="mg-12">
          <el-button
            type="primary"
            size="mini"
            v-if="this.$route.query.sfqr == '0' &&　lcbShow"
            @click="handleLcbqr"
          >里程碑确认</el-button>
        </div>
        ...
        >
          ...
          <el-table-column label="是否确认" width="100">
            <template slot-scope="scope">
              <el-tag
                size="mini"
                :type="scope.row.sfqr == '1' ? 'success' : 'primary'"
                disable-transitions
