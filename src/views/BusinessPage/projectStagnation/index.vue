				...
				<el-table :data="tableData" style="width: 100%" border>
					...
					<el-table-column fixed="left" label="操作" width="220">
						<template slot-scope="scope">
							<el-button v-if="scope.row.canRestart==1" type="text" size="small" @click="handleApply('tz',scope.row)">停滞重启</el-button>
...
export default {
	data() {
		return {
			...
			pageSize:15,
			records:0,
			ztList: [{ mc: ' 流程中', id: 1 }, { mc: '审批通过', id: 2 }, { mc: '审批不通过', id: 3 }],
			formdata: {
				keyword: '',
				ksrq: '',
				jsrq: '',
	...
	methods: {
		...
		handleClick(row,type) {
			this.wid = row.wid;
			if(type == 'process'){
				// this.$router.push({path:'/projectStagnationDetail',query:{wid:row.wid}});
			}else if(type == 'pass'){
