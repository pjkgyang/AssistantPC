<template>
	<div class="pd-10">
				...
				<div class="xxtsqlDetail-filter" v-if="$route.query.lx == 'xms' || $route.query.lx == 'fk'">
					...
					<div>
						...
						<p class="query-list">
							...
							<span v-for="xmzt in xmztMenu" :class="{ 'bg-active': filter.xmzt == xmzt.id }" :key="xmzt.id" @click="handleFilterXMZT(xmzt.id)">{{ xmzt.mc }}</span>
						</p>
					</div>
					<!-- 项目类别 -->
					<div>
						...
						<p class="query-list">
							...
							<span v-for="xmlb in xmlbMenu" :class="{ 'bg-active': filter.xmlb == xmlb.id }" :key="xmlb.id" @click="handleFilterXMLB(xmlb.id)">{{ xmlb.mc }}</span>
						</p>
					</div>
					<div>
					...
					<div v-if="isJZuser != 1">
						...
						<p class="query-list">
							...
							<span v-for="(gcqy, index) in gczdList" :key="index" @click="handleFilterQy(gcqy.label)" :class="{ 'bg-active': filter.qygc == gcqy.label }">
								{{ gcqy.mc }}
							</span>
						</p>
					</div>
		...
		<div><cgcpDialog :show.sync="cgcpShow" :xmbh="xmbh"></cgcpDialog></div>
	</div>
</template>
