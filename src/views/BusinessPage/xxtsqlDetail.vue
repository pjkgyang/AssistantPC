        ...
        >
          ...
          <el-table-column label="问题数" width="100">
            <template slot-scope="scope">
              <a
                href="javaScript:;;"
                @click="handleCheckDetail('wtzs', scope.row.wtzs, scope.row, 'zj')"
              >{{ scope.row.wtzs }}</a>
            </template>
          </el-table-column>
        </el-table>
        ...
        >
          ...
          <el-table-column label="问题数" width="100">
            <template slot-scope="scope">
              <a
                href="javaScript:;;"
                @click="handleCheckDetail('wtzs', scope.row.wtzs, scope.row, 'sh')"
              >{{ scope.row.wtzs }}</a>
            </template>
          </el-table-column>
        </el-table>
        ...
        >
          ...
          <el-table-column label="问题数" width="100">
            <template slot-scope="scope">
              <a
                href="javaScript:;;"
                @click="handleCheckDetail('wtzs', scope.row.wtzs, scope.row, 'gb')"
              >{{ scope.row.wtzs }}</a>
            </template>
          </el-table-column>
        </el-table>
        ...
        <div class="xxtsqlDetail-filter" v-if="$route.query.lx == 'xms' || $route.query.lx == 'fk'">
          <div>
            ...
            <p>
              <el-input
                ...
                style="width:493px;"
                size="mini"
                placeholder="请输入项目编号/项目名称/合同编号/甲方负责人/乙方负责人"
                @change="handleSearchXm"
              ></el-input>&#x3000;
          ...
          <div>
            ...
            <p class="query-list">
              <span
                data-type
                :class="{ 'bg-active': filter.xmzt == '' }"
                @click="handleFilterXMZT('')"
              >全部</span>
              <span
                v-for="xmzt in xmztMenu"
                :class="{ 'bg-active': filter.xmzt == xmzt.id }"
                :key="xmzt.id"
                @click="handleFilterXMZT(xmzt.id)"
              >{{ xmzt.mc }}</span>
            </p>
          </div>
          <!-- 项目类别 -->
          <div>
            ...
            <p class="query-list">
              <span
                data-type
                :class="{ 'bg-active': filter.xmlb == '' }"
                @click="handleFilterXMLB('')"
              >全部</span>
              <span
                v-for="xmlb in xmlbMenu"
                :class="{ 'bg-active': filter.xmlb == xmlb.id }"
                :key="xmlb.id"
                @click="handleFilterXMLB(xmlb.id)"
              >{{ xmlb.mc }}</span>
            </p>
          </div>
          <div>
          ...
          <div v-if="isJZuser != 1">
            ...
            <p class="query-list">
              ...
              <span
                ...
                :key="index"
                @click="handleFilterQy(gcqy.label)"
                :class="{ 'bg-active': filter.qygc == gcqy.label }"
              >{{ gcqy.mc }}</span>
            </p>
          </div>
          <div>
            ...
            <p class="query-list">
              <span
                v-for="qkzt in qkztList"
                @click="handleFilterQk(qkzt.id)"
                :class="{ 'bg-active': filter.qkzt == qkzt.id }"
              >{{ qkzt.mc }}</span>
            </p>
          </div>
        </div>
        <div v-if="$route.query.lx == 'xms'">
          ...
          >
            ...
            <el-table-column label="项目状态" sortable width="110">
              <template slot-scope="scope">
                <el-tag size="mini" type="primary">{{ scope.row.gczt_display }}</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column label="采购产品" min-width="100">
              <template slot-scope="scope">
                <a
                  ...
                  @click="handleCheckCgcp(scope.row.xmbh)"
                >{{ scope.row.cpzs }}</a>
              </template>
            </el-table-column>
