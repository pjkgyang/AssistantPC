    ...
    <layout :title="'周报总览'">
      <header slot="header">
        ...
        <section flex spacebetween>
          <div>
            <span>
              <el-date-picker style="width:138px" :picker-options="pickerBeginDateBefore" @change="handleChooseMonth"
                v-model="monthValue" size="small" type="month" placeholder="选择月" value-format="yyyy-MM" format="yyyy年MM月"
                :clearable="false"></el-date-picker>
            </span>
            <span>
              <el-select v-model="weekValue" size="small" placeholder="请选择" @change="handleChooseWeek" style="width:100px">
                <el-option v-for="item in weeksNum" :key="item" :label="'第' + (item == 1 ? '一' : item == 2 ? '二' : item == 3 ? '三' : item == 4 ? '四' : '五') + '周'"
            ...
            <span v-if="isJzyh != 1">
              ...
              <el-select v-model="ryValue" size="small" placeholder="请选择" @change="handleChoosePreson" style="width:100px">
            ...
            <span>
              ...
              <el-select class="report-chooseitem" style="width:200px" v-model="itemValue" clearable filterable remote
                :remote-method="remoteMethod" @change="changeProject" placeholder="请选择/搜索项目/项目编号" size="small">
                <el-option v-for="item in itemList" :key="item.value" :label="'[' + item.xmbh + '] ' + item.xmmc"
                  :value="item.xmmc"></el-option>
              </el-select>
            </span>
            <span>
              <span class="filter-weight">考核总结：</span>
              <span>
                <el-checkbox v-model="sfkhzj" true-label="1" false-label="0" @change="handleCheckSfzj">是</el-checkbox>
              </span>
            </span>
          </div>
      ...
      <article slot="content">
        ...
        <tableLayout :title="'里程碑工作'">
          <div slot="bottom">
            ...
            <WeekLcbTable @handleClickPz="handleLcbPz" @handleClickEdit="handleLcbEdit" @handleClickCheck="handleLcbCheck"
              @handleCurrentChange="handleLcbCurrentChange" :currentPage="currentWorkPage" :bjWordShow="false"
              :wordShow="wordShow" @handleSizeChange="handleSizeLcbChange" @handleSelectionChange="handleSelectLcb"
              :gznrShow="false" :pageSize="LcbPageSize" :records="LcbRecords" :tableData="LcbList" :isEdit="false"
              :isSelect="true"></WeekLcbTable>
          </div>
        </tableLayout>
        <tableLayout :title="'进度任务'">
          <div slot="bottom">
            ...
            <processTable :tableData="processList" @handleClickPz="handleProcessPz" @handleClickEdit="handleProcessEdit"
              @handleClickCheck="handleProcessCheck" @handleCurrentChange="handleJdCurrentChange" @handleSizeChange="handleSizeJdChange"
              @handleSelectionChange="handleSelectJd" :isSelect="true" :pageSize="pageSize" :records="JdRecords"
              :currentPage="currentProcessPage" :isPz="true" :deleteShow="false" :bjWordShow="true" :isEdit="false"
              :wordShow="wordShow"></processTable>
          </div>
        </tableLayout>
        <tableLayout :title="'问题处理'">
          <div slot="bottom">
            ...
            <MonthWeekQuestionTable :tableData="questionList" @handleClickEdit="handleQuestionEdit"
              @handleCurrentChange="handleQuestionPageChange" @handleClickPz="handleQuestionPz" @handleClickCheck="handleQuestionCheck"
              :bjWordShow="true" :othShow="othShow" @handleSizeChange="handleSizeWtChange" @handleSelectionChange="handleSelectWt"
              :pageSize="WtPageSize" :records="WtRecords" :currentPage="currentQuestionPage" :isSelect="true" :isEdit="false"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 导出
      handleExport() {
        window.open(window.baseurl + 'plan/exportWeekPlan.do?qygc=' + this.qyValue + '&month=' + this.monthValue +
          '&zxh=' + this.weekValue + '&yhbh=' + this.ryValue);
      },
      // 新增项目周报
      handleClickAddzb() {
        this.jhzdShow = true;
      ...
      initializeDate() {
        ...
        if (new Date(lastDay).getTime() >= new Date(GetDateStr(0)).getTime() && new Date(GetDateStr(0)).getTime() > new Date( weekEndDate).getTime()) {
          ...
          this.weeksNum = week; //周数
          this.weekValue = this.curWeek = weekNumInMonth(this.year + '/' + ((this.month + 1) < 10 ? '0' + (this.month + 1) : this.month + 1));
