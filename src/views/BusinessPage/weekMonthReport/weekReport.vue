<template>
	<div>
			...
			<header slot="header">
				...
				<section flex spacebetween>
					<div>
						...
						<span><el-input placeholder="请输入内容" v-model="weekDay" :disabled="true" size="small" style="width:200px"></el-input></span>
		...
		<div class="week-report-oprater" v-if="false"><el-button type="primary" size="small" @click="handleReaded">已阅</el-button></div>
...
export default {
	data() {
		return {
			...
			wtztList: [{ value: '0', label: '处理中' }, { value: '2', label: '已申请关闭' }],
			ycztList: [{ value: '', label: '全部' }, { value: '0', label: '超期' }, { value: '1', label: '未超期' }],
			ycztValue: '',
			wtztValue: [],
			monthValue: '',
			sfkhzj:'1',//是否考核总结
	...
	methods: {
		...
		initializeDate() {
			...
			if (
				new Date(NowDate).getTime() >= new Date(weekStartDate).getTime() &&
			...
			// else if (new Date(NowDate).getTime() >= new Date(nextWeekStartDate).getTime() && new Date(NowDate).getTime() <= new Date(nextWeekEndDate).getTime()) {
			// 	this.weekValue = 1;
