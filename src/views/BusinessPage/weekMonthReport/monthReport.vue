     ...
     <layout :title="'月度计划总览'">
        <header slot="header">
          ...
          <section flex spacebetween >
            <div>
                <span><el-date-picker @change="changeMonthDate" v-model="monthValue"  format="yyyy年MM月" value-format="yyyy-MM" size="small" type="month" placeholder="选择月"></el-date-picker></span>&#x3000;
        ...
        <article slot="content" style="position:relative">
            <tableLayout :title="'月度工作'+nextTitle">
             ...
             <div slot="bottom">
                <section class="month-plan-condition month-hwys">
                 ...
                 <section class="month-plan-condition" >
                ...
                <MonthTable :tableData="monthWorkList" :otherShow="otherShow && isJzuser != 1"  :ispz="true" :isEdit="false" @handleClickPz="handleYdjhPz" @handleClickEdit="handleYdjhEdit" @handleCurrentChange="handleYdjhChange"
            ...
            <tableLayout :title="'月度问题处理'+nextTitle">
             <div slot="bottom">
                ...
                <MonthWeekQuestionTable :tableData="monthQuestionList" @handleClickPz="handleQuestionPz" @handleClickEdit="handleQuestionEdit" @handleClickCheck="handleQuestionCheck" 
                @handleSizeChange="handleSizeWtChange" @handleSelectionChange="handleSelectWtChange" @handleCurrentChange="handleQuestionPageChange" :isSelect="true" :ideleteShow="!isBlocked" 
                :othShow="otherShow" :isEdit="!isBlocked && groupTag.indexOf('JYGL') == -1" :pageSize="questionPageSize" :records="QuestionRecords" :currentPage="questionPage"></MonthWeekQuestionTable>
             </div>  
            </tableLayout>
        </article>
     </layout>
...
const yfcContent = "<h5 style='font-weight:700'>本月计划 <span style='color:#f00'>已制定，已封存。</span></h5>"
 export default {
   data () {
     return {
            keyword:'',
            ...
            detailTitle:'',
            detailType:{},
            breadList:[{title:'月度计划总览',url:'/businesspage/monthReport'},{title:'',url:''}],
            monthWorkList:[],
            monthQuestionList:[],
            monthValue:'',
            gczdList:[],
   ...
   methods:{
      ...
      remoteMethod(val) {  //远程搜索
       ...
       handleEditofBatch(){             // 编辑（批量）计划
            this.Wid = '';
            this.planType = 1
            this.isPlanEdit = true;
            this.bjShow = true;
       },
       handlePzofBatchWt(){             // 批注（批量）问题      
            this.Wid = '';
            this.planType = 2;
            this.isPlanPz = false;
            this.pzlsShow = false;
       ...
       handleEditofBatchWt(){           // 编辑（批量）问题
        ...
        handleSelectZt(val){           // 状态
            this.getMonthWorkList(1);
            this.countMonthWorkWglg(); // 合计
            this.currentPage = 1;
        },
        handleYdjhChange(data){ // 月工作计划 分页
            this.currentPage = data;
            this.getMonthWorkList(data);
        },
    //    ********************

        handleYdjhPz(data){      // (计划)批注
            this.pzlsShow = true;
            this.pzShow = true
            this.planType = 1
        ...
        handleYdjhEdit(data,index){    // (计划)编辑
            this.index = index;
            this.planType = 1;
            this.Wid = data.ygzWid;
        ...
        handleSelectionChange(data){         // 多选 里程碑状态
            this.multipleSelection = data
        },
        handleSelectWtChange(data){          // 多选 问题
            this.multipleSelectionWt = data
        },
        handleSizeChange(data){              // 每页条数 计划
           this.workPageSize = data; 
           this.getMonthWorkList(1);
        },
        handleSizeWtChange(data){            // 每页条数 问题
        ...
        handleQuestionPageChange(data){      // 分页
        ...
        handleClickPzSure(data){     // 批注(确定)
            if(this.isPlanPz){
                this.addComment(data,this.Wid?this.Wid:this.multipleSelection.join(','));
            }else{
                this.addComment(data,this.Wid?this.Wid:this.multipleSelectionWt.join(','));
            }
        },

        getMonthWorkList(curPage){  //获取月工作计划列表
            pageMonthWork({
                curPage:curPage,
        ...
        getMonthQuestionList(curPage){  //获取月问题计划列表
            pageMonthQuestion({
                curPage:curPage,
        ...
        saveMonthWork(wwcyy,hxcs){  // 保存月工作计划
          saveMonthWork({
              wid:this.Wid,
              qygc:this.qyValue,
              yf:this.monthValue,
        ...
        saveMonthQuestion(wwcyy,hxcs){ // 保存月问题
            saveMonthQuestion({
              wid:this.Wid,
              qygc:this.qyValue,
              yf:this.monthValue,
