  ...
  methods: {
      ...
      allowSaveWeekPlan({
        ...
        if (data.state == "success") {
          if (!!data.data) {
            ...
            this.$alert("未完成原因、后续措施为必填项，请填写", "提示", {
              confirmButtonText: "确定",
              type: "warning",
              callback: action => {}
            });
    ...
    handleAdded(param) {
      ...
      if (param == "rwjd-bz" || param == "rwjd-xz") {
        ...
        this.wtShow = true;
        if (param == "wtcl-xz") {
          this.cnjssj = this.getWeekDate(
            this.NextMonth.split("-")[0],
            this.NextMonth.split("-")[1] - 1,
            this.nextWeeksNum
          ).split("至")[1];
        } else {
    ...
    // ************ 下周
    handleXzLcbEnterSearch() {
      // 下周里程碑（关键字）
      this.pageWeekWork(1, this.NextMonth, this.NextweekValue, true);
      this.currentNextLcbPage = 1;
    },
    handleChangeXzLcbzt(val) {
      // 下周里程碑（状态）
    ...
    handleXzWtEnterSearch() {
      // 下周问题（关键字）
      this.pageWeekQuestion(1, this.NextMonth, this.NextweekValue, true);
      this.currentNextWtPage = 1;
    },
    handleSelectXzWtzt(val) {
      // 下周问题状态
      this.pageWeekQuestion(1, this.NextMonth, this.NextweekValue, true);
      this.currentNextWtPage = 1;
    },
    ...
    handleDeletejd(data, isNextWeek) {
      // （进度）删除
    ...
    handleDeleteWt(data, isNextWeek) {
      //删除 问题
    ...
    handleDeleteLcb(data, isNextWeek) {
      // （里程碑）删除
    ...
    addWeekQuestionPlan(wids, month, zxh) {
      // 添加周问题
      addWeekQuestionPlan({
        wids: wids,
        month: month,
    ...
    addWeekWorkPlan(lcbbhs, month, zxh) {
      // 添加里程碑工作
      addWeekWorkPlan({
        lcbbhs: lcbbhs,
        yhbh: this.yhbh,
    ...
    pageWeekWork(curPage, month, zxh, isNext) {
      //获取周工作计划的列表
      pageWeekWork({
        curPage: curPage,
    ...
    pageWeekQuestion(curPage, month, zxh, isNext) {
      //获取周问题计划的列表
      pageWeekQuestion({
        curPage: curPage,
    ...
    pageWeeklyReport(curPage, month, zxh, isNext) {
      //获取周进度计划的列表
      pageWeeklyReport({
        curPage: curPage,
    ...
    isWeekPlanBlocked(month, zxh, isNextWeek) {
      // 是否封存
      isWeekPlanBlocked({
        month: month,
        zxh: zxh
    ...
    intelizeDate(year, month) {
      ...
      this.bzJssj = this.getWeekDate(year, month - 1, this.weeksNum).split(
        "至"
      )[1];
      this.nextWeeksNum = getWeeks(
        this.NextMonth.split("-")[0],
        this.NextMonth.split("-")[1]
      );
      this.nextWeekDay = this.getWeekDate(
        this.NextMonth.split("-")[0],
        this.NextMonth.split("-")[1] - 1 == 0
          ? 0
          : this.NextMonth.split("-")[1] - 1,
        this.NextweekValue
      );
