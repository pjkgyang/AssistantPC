<template>
  <div>
    ...
    <section class="Project_detail" v-if="!show">
      <!-- 项目分类 -->
      <div
        class="Project_detail_highrisk"
        v-for="(item,key,index) in myItemData"
      ...
      <div>
        <div class="project_more_choose">
          <div>
            ...
            <el-input
              class="search-input"
              placeholder="请输入项目编号/项目名称/合同编号/项目经理"
          ...
          <div v-if="showList">
            <p>
              ...
              <span
                v-for="xmbq in xmbqMenu"
                :data-xmbq="xmbq.id"
                :class="{'bg-active':xmbq.id == xmbqC}"
                :key="xmbq.id"
                @click="CheckXmbq"
              >{{xmbq.mc}}</span>
            </p>
