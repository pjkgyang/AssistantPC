<template>
	<div class="task-log">
		<div class="task-log-tabel">
			<div v-if="!!$route.query.xmbh">
				<h4>【{{ $route.query.xmbh }}】&#x3000;{{xmmc}}</h4>
			</div>
			<el-table :data="tableData" border style="width: 100%">
				<el-table-column label="操作" width="120" v-if="!$route.query.xmbh">
				...
				<el-table-column label="阅读状态" width="90" v-if="!$route.query.xmbh">
					<template slot-scope="scope">
						<div slot="reference">
							<span title="点击设为已阅" class="tasklog-ydzt tasklog-ydzt-wy" v-if="scope.row.ydzt == 0" @click="handleReadlog(scope.$index, scope.row)">
								{{ scope.row.ydzt_display }}
							</span>
							<span :class="{ 'tasklog-ydzt': true, 'tasklog-ydzt-yy': scope.row.ydzt == 1 }" v-if="scope.row.ydzt != 0">
								{{ scope.row.ydzt == 1 ? '已阅' : scope.row.ydzt_display }}
							</span>
						</div>
					</template>
				</el-table-column>
				<el-table-column prop="cjrxm" label="填写人" width="90"></el-table-column>
				<el-table-column v-if="!$route.query.xmbh" prop="xmbh" label="项目编号" show-overflow-tooltip sortable width="110"></el-table-column>
				<el-table-column v-if="!$route.query.xmbh" prop="xmmc" label="项目名称" show-overflow-tooltip min-width="220"></el-table-column>
				<el-table-column prop="gcms" label="内容" show-overflow-tooltip min-width="300"></el-table-column>
				<el-table-column prop="cpmc_display" label="产品名称" show-overflow-tooltip min-width="180"></el-table-column>
				<el-table-column prop="rwmc_display" label="任务名称" show-overflow-tooltip min-width="150"></el-table-column>
		...
		<el-dialog :visible.sync="tasklogDetailVisible" title="日志详情" width="650px" :close-on-click-modal="false">
			<div class="tasklog-detail">
				...
				<p>
					<span>产品名称 :</span>
					<span>{{ taskDetail.cpmc_display == '' ? '无' : taskDetail.cpmc_display }}</span>
				</p>
				<p>
					<span>关联任务 :</span>
					<span>{{ taskDetail.rwmc_display == '' ? '无' : taskDetail.rwmc_display }}</span>
				</p>
				<p>
					<span>工时(小时) :</span>
				...
				<p>
					...
					<span class="name-wrapper">
						<span v-for="(fj, index) in taskDetail.fjData" :key="index" :data-fj="fj.fjbh" @click="handleDownFile('', '', $event)">{{ fj.fjmc }}</span>
					</span>
				</p>
...
export default {
	...
	mounted() {
		...
		delete this.paramsData.xmmc;
		this.getPersonalWorkLog(this.paramsData);
	},
	methods: {
		...
		getPersonalWorkLog(obj) {
			// 获取个人日报 (任务过程)
			this.$get(this.API.getPersonalWorkLog, obj).then(res => {
