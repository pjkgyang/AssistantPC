<template>
  <div style="height:calc(100% - 50px);overflow-x:hidden">
    <div style="position:fixed;width:100%;z-index:100;">
      <user-banner
        ...
        :fbdetail="true"
        :userNum="itemUserNum"
        @handleItemUser="handleItemUser"
        @handleTabsClick="handleTabsClick"
      >
        <span
          class="project_task_itemInfo"
          slot="xmmc"
          :title="'['+xmkbInfo.xmbh+']'+xmkbInfo.xmmc"
        >{{'['+xmkbInfo.xmbh+']'+xmkbInfo.xmmc}}</span>
      </user-banner>
      <div style="position:absolute;right:20px;top:40px;z-index:100;" v-if="userListShow">
          ...
          <ul class="task_list">
            <div>
              <li
                ...
                :key="index"
                :data-fbcp="task.cp+'&'+task.cpbh"
                @click="getTaskinfo($event,index)"
              >
                <p>
                  <strong
                    :title="task.cp"
        ...
        >
          <div>
            ...
            <div class="task-detail-scenario-infos-wrap">
              <p>
                <span style="display:inline-block;width:90px" class="el-icon-service">责任人</span>
              ...
              <p>
                <span style="display:inline-block;width:90px" class="el-icon-news">任务状态</span>
                <span>{{lcbTasks.zt_display}}</span>
              </p>
              <p>
                <span style="display:inline-block;width:90px" class="el-icon-sort">优先级</span>
                <span>{{lcbTasks.yxj_display}}</span>
              </p>
              <p>
                <span style="display:inline-block;width:90px" class="el-icon-time">工时</span>
                <span>{{lcbTasks.jhgs}}&#x3000;</span>
              </p>
              <p>
                <span style="display:inline-block;width:90px" class="el-icon-document">备注</span>
            ...
            <div class="detail-activities-wrap">
              ...
              <div class="activities-list-wrap">
                ...
                >
                  <a
                    v-if="(taskProcessNum - curPageNum) > 0"
                    @click="showAllprocess"
                  >显示较早的 {{taskProcessNum - curPageNum}} 条动态</a>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  mounted() {
    ...
    document.title =
      "项目详情-" + "[" + this.xmkbInfo.xmbh + "]" + this.xmkbInfo.xmmc;
  ...
  methods: {
    ...
    // 获取产品 里程碑
    getTaskinfo(e, params) {
      this.fbcpmc = e.currentTarget.getAttribute("data-fbcp").split("&")[0];
      this.fbcpbh = e.currentTarget.getAttribute("data-fbcp").split("&")[1];
