...
<div style="height:calc(100% - 50px);overflow-x:hidden">
  <div style="position:fixed;width:100%;z-index:100;">
    <user-banner :shown="false" :xmdetail="false" :Operatepower="Operatepower"  :bannerACtive="tabsLabel" :itemMc="'['+xmkbInfo.xmbh+']'+xmkbInfo.xmmc" :fbdetail="true" :userNum="itemUserNum"  @handleItemUser="handleItemUser" @handleTabsClick="handleTabsClick">
      <span class="project_task_itemInfo" slot="xmmc" :title="'['+xmkbInfo.xmbh+']'+xmkbInfo.xmmc">{{'['+xmkbInfo.xmbh+']'+xmkbInfo.xmmc}}</span>
    </user-banner>
  ...
  <div class="project_task_detail" @click.stop="selectHide">
    ...
    <div  class="project_task_detail_taskbar">
          <div class="project_info">
            ...
            <ul class="task_list" >
              <div>
                <li v-if="taskList.length != 0 && taskList != null" :class="{'list-active':shownindex === index}" v-for="(task,index) of taskList" :key="index" :data-fbcp="task.cp+'&'+task.cpbh" @click="getTaskinfo($event,index)">
                  <p>
                    <strong :title="task.cp" :class="{'on_active':shownindex === index,'project_task_detail_cpmc':true}">{{task.cp}}</strong><br>
...
import { getTasksByCatalog ,addOrUpdateTask ,getTasks,getTaskDto ,getTaskLogs ,deleteTask ,changeTaskStatus} from '@/api/task.js'
import { addOrUpdateTaskProcess ,queryTaskProcess ,getChildrenByLcbms ,getTemplateByRwbh} from '@/api/TaskProcess.js'
import { querytaskParticipant ,queryUser ,queryProjectParticipant , addTaskParticipant,deleteTaskParticipant ,modifyTaskLeader ,queryProductParticipant} from '@/api/personal.js'
...
export default {
  ...
  mounted(){
      ...
      document.title = '项目详情-'+'['+this.xmkbInfo.xmbh+']'+this.xmkbInfo.xmmc
