    ...
    <el-table
      ...
      <el-table-column fixed="left" label="操作" width="150" v-if="selectShow">
        <template slot-scope="scope">
          ...
          <span v-if="isDelete"><el-button style="color:red"  type="text" size="small"  @click="handleClickDelete(scope.row,scope.$index)">删除</el-button></span>
          ...
          <span v-if="bjWordShow"><span class="table-hr" v-if="isDelete || ispz"> 丨 </span><el-button  type="text" size="small"  @click="handleClickEdit(scope.row,scope.$index)">{{isZj?'总结':'计划'}}</el-button></span>
          <span><span class="table-hr" v-if="isDelete || ispz ||bjWordShow"> 丨 </span><el-button @click="handleClickCheck(scope.row)" type="text" size="small">查看</el-button></span>
        </template>
      </el-table-column>
