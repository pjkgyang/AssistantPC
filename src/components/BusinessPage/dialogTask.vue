        ...
        <div class="search-dialog-task" flex >
         ...
         <div class="choose-daily-alltask" >
             <el-collapse v-model="activeNames" v-if="shown" @change="handleCollapse" accordion>
                <!-- <el-collapse-item v-for="(collapse,i) in collapseList" :title="collapse" :name="i">
                   <ul>
                        <li class="choose-daily-item" v-for="(item,index) in dataList" :title="item.xmmc" :data-xmmc="item.xmmc"  :data-xmbh="item.xmbh" @click="handleNext">{{item.xmmc}}</li>
                        <li class="choose-daily-more"><a href="javascript:void(0)" @click="handleMoreitem(index)">加载更多</a></li>
                ...
                <!-- <el-collapse-item title="高风险" name="2"  >
                   <ul>
                        <li class="choose-daily-item" :title="item.xmmc" v-for="(item,index) in riskData" :data-xmmc="item.xmmc"  :data-xmbh="item.xmbh" @click="handleNext">{{item.xmmc}}</li>
                        <li class="choose-daily-more"><a href="javascript:void(0)" @click="handleMoreitem(2)" v-if="gfxCurPage < gfxTotal && gfxTotal > 1">加载更多...</a></li>
                        <li class="choose-daily-emptyItem"  v-if="riskData.length == 0">暂无项目</li>
                    </ul>
                </el-collapse-item> -->
...
<script>
import {queryXbxm,queryGfxxm,queryWyyxm,queryWcyxm,queryGbxm,queryAllxm,getMyProjects,getProjects} from '@/api/xmkb.js'
...
export default {
  data(){
      return{
            collapseList:['收藏项目','高风险','我负责的项目','我参与项目','已关闭项目'],
            dataList:[],
            taskGroup:false,
            taskList:false,
            xbData:[],
  ...
  methods:{
      ...
      handleCollapse(data){ //折叠面板
            if(!data) return;
            this.curPage = 1
            this.getMyProjects(data);         
      },
      handleMoreitem(param){  // 查看更多项目
        switch(param){
            case 1:
             this.xbCurPage += 1;
