<template>
  <div class="edit-task-dialog" @click="selectHide">
    <div class="task-detail-scenario-infos-wrap">
      <div>
        <span class="el-icon-service"> 责任人</span>
          ...
          <div style="float:left;">
            ...
            <span v-if="!jfshown">
              <el-select style="width:200px" v-model="cpInfo.jfzrr" filterable size="mini" placeholder="请选择/搜索责任人">
                <el-option v-for="(item,index) in options" :key="index" :label="item.userName" :value="item.userName+'&'+item.userId">
                </el-option>
              </el-select>
            </span>
            <span>
              <el-button size="mini" @click="changeCPJfzrr">{{this.jfshown==true?'修改':'取消修改'}}</el-button>
            </span>
            <span v-if="!jfshown">
              <el-button size="mini" @click="CommitChangeCPJfzrr">确定修改</el-button>
            </span>
          </div>
          <div style="float:left">
            ...
            <span>
              <el-button size="mini" @click="changeCPYfzrr">{{this.yfshown==true?'修改':'取消修改'}}</el-button>
            </span>
            <span v-if="!yfshown">
              <el-button size="mini" @click="CommitChangeCPYfzrr">确定修改</el-button>
            </span>
          </div>
        </div>
      </div>
    ...
    <div class="involve-view">
      ...
      <div>
        <div flex flexwrap>
          <el-tag  style="margin-bottom:4px" :key="index" v-for="(tag,index) in dynamicTags" :closable="tag.del == true" :disable-transitions="false" @close="handleClose(tag)">
            {{tag.userName}}
          </el-tag>
  ...
  methods: {
    ...
    addTaskParty(e) {
      ...
      this.dynamicTags.forEach((ele, i, arr) => {
        userArr.push(ele.userName);
      });
      if (!userArr.includes(yhxm)) {
        // 增加产品参与者
        addProductParticipant({
          xmbh: this.xmbh,
          cpbh: this.cpinfo.cpbh,
...
.task-detail-scenario-infos-wrap > div input,
.task-detail-scenario-infos-wrap > div div {
  width: 82%;
  padding: 3px;
  outline: none;
  color: #666;
