		...
		<div class="project-question">
			<div>
				<ul class="project-question-detail">
					...
					<li>
						...
						<div class="project-question-detail-bottom"><div v-html="qusetionInfo.nr"></div></div>
					</li>
									...
									reply.nr +
										(!reply.sm
											...
											: '<br><br><span style=color:red>' +
											  ...
											  '</span>')
								"
								v-if="reply.hflx != 9 && reply.hflx != 14"
							></div>
			...
			<el-dialog width="600px" title="问题评价" :visible.sync="innerVisible" :close-on-click-modal="false" append-to-body>
				<div class="question-evaluate">
					...
					<div class="question-rate" v-if="fwzlValue <= 3">
						...
						<p><el-input type="textarea" :rows="2" placeholder="请输入说明内容" v-model="cpsm" style="width:490px;"></el-input></p>
					</div>
						...
						<el-table :data="ContributionPeople" style="width: 100%" border :max-height="580" border>
							...
							<el-table-column label="工时(小时)">
								<template slot-scope="scope">
									<el-input
										v-model="scope.row.qrgs"
										:class="{ 'is-outline': questionReply.length != 0 && questionReply[0].sfhf == 1 }"
										size="mini"
										:readonly="questionReply.length != 0 && questionReply[0].sfhf != 1"
									></el-input>
								</template>
							</el-table-column>
						</el-table>
					...
					<div style="text-align:right;margin:10px 0;"><el-button type="primary" size="mini" @click="commitQuestion">提交</el-button></div>
				</div>
			</el-dialog>
			...
			<el-dialog :title="zdTitle" :visible.sync="zdfzrVisible" :close-on-click-modal="false" width="800px" top="30px" @close="ClosezdfzrVisible" append-to-body>
				<div style="padding:10px;">
					<zdfzrChoose
						:lchjList="lchjList"
						:visible="zdfzrVisible"
						:tableData="tableData"
						:zdTitle="zdTitle"
						:jkrTotal="jkrTotal"
						@handleYYRYLX="handleYYRYLX"
						@handleClose="handleClose"
						@handleCommitZdr="handleCommitZdr"
						@queryComplainZRR="queryComplainZRR"
						@queryComplainUser="queryComplainUser"
						@handleCurrentChange="handleZrrChange"
					></zdfzrChoose>
				</div>
			</el-dialog>

			<el-dialog title="申请关闭" :visible.sync="sqjsVisible" :close-on-click-modal="false" width="800px" top="30px" append-to-body>
				<div style="padding:10px;"><sqjsForm :gs="sqgbgs" @submitForm="submitForm" @closeForm="closeForm"></sqjsForm></div>
			</el-dialog>
			...
			<el-dialog title="提报crowd任务" :visible.sync="crowdVisible" :close-on-click-modal="false" width="1000px" top="30px" append-to-body>
				<div style="padding:20px 10px ">
					...
					<div v-if="crowdType == 1">
						...
						<p style="text-align:right;padding:10px 0"><el-button type="primary" size="small" @click="handleCommitCrowd">提交</el-button></p>
					</div>
					<div v-if="crowdType == 2" class="send_crowd">
						<el-form :model="crowdxqData" :rules="rules" ref="crowdxqData" class="demo-ruleForm">
							<el-form-item label=""><div class="send-crowd_topic">发布需求流程</div></el-form-item>
							...
							<el-form-item><div class="send-crowd_topic">请完善需求的基本信息</div></el-form-item>
							...
							<!-- 需求描述 -->
							<el-form-item prop="xqms"><el-input type="textarea" placeholder="请输入需求描述" v-model="crowdxqData.xqms"></el-input></el-form-item>

							<el-form-item><uploadFile :isCrowd="true" @handleUploadFile="handleUploadFile"></uploadFile></el-form-item>

							<el-form-item><div class="send-crowd_topic">请选择需求客户/学校/个人的期望交付日期，该日期仅作数据记录使用</div></el-form-item>
							<el-form-item>
								<el-date-picker
							...
							<el-form-item>
								<el-date-picker
									@change="handleChoosejfDate"
									value-format="yyyy-MM-dd"
									:picker-options="pickerJfrqDateBefore"
									size="mini"
									type="date"
									placeholder="请输入需求招标截至日期，到达该日期后，如果没有投标或者选标，需求则会自动关闭"
									v-model="crowdxqData.zbjzrq"
									style="width: 100%;"
								></el-date-picker>
							</el-form-item>
							<el-form-item>
							...
							<el-form-item><div class="send-crowd_topic">请选择需求的相关项目信息</div></el-form-item>
							<el-form-item>
                  ...
                  <el-select  size="mini"  v-model="crowdxqData.fbbh" placeholder="请选择分包信息">
							...
							<el-form-item text-right><el-button size="mini" type="primary" @click="sendSubmitForm('crowdxqData')">提交</el-button></el-form-item>
						</el-form>
					</div>
				</div>
			</el-dialog>
			...
			<el-dialog title="编辑回复内容" :visible.sync="editEplyVisible" :close-on-click-modal="false" width="800px" top="30px" append-to-body>
				<div style="padding:20px 10px" class="edit-reply">
					...
					<div style="text-align:right"><el-button type="primary" size="mini" @click="handleSaveReplyContent">保存</el-button></div>
				</div>
			</el-dialog>
			<el-dialog title="申请待验证" :visible.sync="sqdyzVisible" :close-on-click-modal="false" width="600px" top="30px" append-to-body>
				<div style="padding:20px 10px" class="question-sqdyz">
					...
					<div style="text-align:right"><el-button size="mini" type="primary" @click="saveSqdyz">保存</el-button></div>
				</div>
			</el-dialog>
			...
			<twDialog
				...
				:guid="gsValue"
				@handleSLsuccess="handleSLsuccess"
			></twDialog>
			<!-- 发送运维消息 -->
...
import { GetQuestionDateStr, formatTime, getMenu, getSession, getMyDate, compare, EventBus } from '@/utils/util.js';
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				rwfl: [{ required: true, message: '请选择需求分类', trigger: 'change' }],
	...
	methods: {
		...
		handleSaveReplyContent() {
			...
			this.replySet(this.questionReply[this.isEditReplyIndex].wid, $('#summernoteEditReplay').summernote('code'), false, this.replygs);
		},
		//删除回复
		...
		handleChange(file, fileList) {
			...
			for (var i = 0, l = this.filesData.length; i < l; i++) {
				for (var j = i + 1; j < l; j++) if (this.filesData[i].name === this.filesData[j].name) j = ++i;
				...
				if (data.state == 'success') {
					...
					if (data.data != null && data.data.length != 0) {
						this.questionReply.forEach((ele, i, arr) => {
							try {
								ele.nr = decodeURIComponent(ele.nr).replace('/gcgl/sys/emapcomponent/file/getFileByToken/', '/emap/sys/emapcomponent/file/getFileByToken/');
							} catch (err) {
								ele.nr = ele.nr;
							}
						});
