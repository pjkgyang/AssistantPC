    ...
    <ul class="project-question-list">
      <li v-for="(question, index) in questionList">
        <section class="spacearound colcenter">
          ...
          <div class="question-info">
            ...
            <span>
              ...
              &#x3000;
              <span
                style="color:#363748;font-size:12px;"
              >问题编号：{{ question.wtbh }}</span>
              &#x3000;
              <span
              ...
              >{{ question.gjzt == 0 ? '未纳入计划' : question.gjzt == 1 ? '计划中' : '计划已改进' }}</span>
            </span>
            &#x3000;
            ...
            <div v-if="wtbqShow">
              <span>
                ...
                <span
                  v-if="!!question.bqMc"
                  v-for="bq in question.bqMc.split(',')"
                  class="question-info-wtbq"
                >{{ bq }}</span>
          ...
          <div class="question-state">
            <span
              ...
              :class="{ 'ygb-color': question.fbzt == 1, 'clz-color': question.fbzt != 1 }"
            >{{ question.lcMc }}</span>
            <br />
            ...
            <!-- !bjscShow && gjjhShow &&  -->
            <span
              v-if="(!!question.fbqdm && (question.fbqdm.includes('W03') && groupTag.includes('NLPXZJ')) || (question.fbqdm.includes('W05') && groupTag.includes('ZJ')))"
            >
        ...
        <section
          ...
          class="pull-right"
          style="width:15%"
          v-if="question.sqgbCount > 0 && question.fbzt != 1 && username == question.fbrxm && isShow"
        >
		...
		<section
          ...
          class="pull-right"
          style="width:15%"
          v-if="question.sqgqz  > 0 && question.fbzt != 1 && username == question.fbrxm && isShow"
        >
          <el-button type="primary" size="mini" @click="handleAgreeGq(question, index)">同意</el-button>
          <el-button type="danger" size="mini" @click="handleRejectGq(question, index)">驳回</el-button>
        </section>
...
export default {
  ...
  methods: {
	...
	handleAgreeGq(params, index){
	  this.$emit("handleAgreeGq", params, index);
	},
	// 驳回挂起
	handleRejectGq(params, index){
      this.$prompt("请输入驳回说明", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        inputType: "textarea",
        ...
        inputErrorMessage: "请输入驳回说明（必填）"
          ...
          this.$emit("handleRejectGq", params, index, value);
      }).catch(() => {});
	},
    editQuestion(params) {
