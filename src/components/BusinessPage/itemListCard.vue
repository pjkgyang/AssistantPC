<template>
    <div>
        <ul v-if="!show" class="project-xmkb">
            <li v-for="(data,index) in datas" :key="index"  v-if="datas.length != 0" @click="handlePage(data)">
                        ...
                        <div class="project-grid-other">
                            <p class="project-grid-card_projectLeader">
                                ...
                                <span v-if="data.yssj != null &&　data.xmjd == '建设中'">
                                    <span v-if="new Date(data.yssj).getTime() < new Date().getTime()" class="sf-overdue" :class="{'overdue-active':new Date(data.yssj).getTime() < new Date().getTime()}">{{new Date(data.yssj).getTime() < new Date().getTime()?'已超期':'未超期'}}</span> </span> <span style="font-size: 12px;color:#363748;">{{data.yssj == ''?'无':data.yssj}}</span>
                            </p>
