<template>
  <div @click="selectHide">
      <div style="height:540px;">
          ...
          <div style="padding:0 20px 0 0;border-bottom: 1px solid #ccc; ">
...
<script>
import {  queryUser ,queryProjectParticipant , addTaskParticipant,deleteTaskParticipant ,modifyTaskLeader} from '@/api/personal.js'
export default {
  data(){
    return{
         listt0: [],
  ...
  methods:{
     ...
     selectHide(e){
      if(e.target.className == 'el-input__inner' && e.target.getAttribute('placeholder') == '搜索姓名'){
...
.task-gs-date::before,.task-gs-date::before{
  content: '*';
  color: #f56c6c;
  padding: 0 2px;
}
