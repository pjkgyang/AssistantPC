		...
		<table class="table table-xjpmCp" border>
			...
			<tbody>
				<tr v-for="(table, i) in [...tableData].splice(1, limit - 1)" :key="i" v-if="tableData.length != 0">
					...
					<td>
						<a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh, 'star1', table.star1)">{{ table.star1 }}</a>
					</td>
					<td>
						<a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh, 'star2', table.star2)">{{ table.star2 }}</a>
					</td>
					<td>
						<a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh, 'star3', table.star3)">{{ table.star3 }}</a>
					</td>
					<td>
						<a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh, 'star4', table.star4)">{{ table.star4 }}</a>
					</td>
					<td>
						<a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh, 'star5', table.star5)">{{ table.star5 }}</a>
					</td>
					<!-- <td><a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh,'staravg',table.staravg)">{{table.staravg}}</a></td> -->
					<td>{{ table.starbad }}</td>
					<td>{{ table.staravg }}</td>
					<td>
						<a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh, 'wtgbs', table.wtgbs)">{{ table.wtgbs }}</a>
					</td>
					<td>
						<a href="javaScript:void(0)" @click="handleWtpfCp(table.cpxbh, 'wtzs', table.wtzs)">{{ table.wtzs }}</a>
					</td>
					<!-- <td>{{table.xmtdgs}}</td>
                <td>{{table.gczjgs}}</td>
                <td>{{table.cpzjgs}}</td> -->
