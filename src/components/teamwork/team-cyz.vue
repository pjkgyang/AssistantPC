	...
	<div class="item-addUser">
		...
		<div class="item-user-outTable">
			<table class="item-user-table">
				...
				<tr v-for="(cyz, index) in itemCyz" v-if="itemCyz.length" :key="index">
					...
					<td>
						<el-button v-if="cyz.roleName.indexOf('销售') != -1 && (userTag || itemYfzrr.userName == username)" plain size="mini" plain @click="changeXsr(cyz)">
							修改
						</el-button>
			...
			<ul class="item-user-list">
				<li style="position:relative" flex>
					<el-input
						v-model="keyword"
						size="mini"
						:style="{ width: isAddCyz && itemJfzrr.userId != userId && isJzuser != 1 ? '300px' : '655px' }"
						:class="{ 'isAddCyz-cyz': isAddCyz, 'isAddCyz-jf': !isAddCyz }"
						placeholder="搜索人员姓名/编号"
						@change="searchUser"
					></el-input>
					<!-- 项目甲方不显示 -->
					<span style="width:40%" v-if="isAddCyz && itemJfzrr.userId != userId && isJzuser != 1">
						人员类别:
				...
				<li v-for="(user, index) in UserList" v-if="UserList.length != 0" :key="index">
					...
					<span>
						<el-button :disabled="user.state == 'ytj'" size="mini" v-if="(isAddCyz && jszbr) || mark" @click="addItemCyz(user, index)">
							{{ user.state == 'ytj' ? '已添加' : '添加' }}
						</el-button>
				...
				<div v-if="UserList.length == 0" text-center class="pd-10">暂无人员数据</div>
			</ul>
			<div style="padding:10px 0;text-align:center"><pagination :pageSize="pageSize" :total="total" @handleCurrentChange="handleCurrentChange"></pagination></div>
		</el-dialog>
		...
		<el-dialog width="750px" title="添加外部人员" :visible.sync="innerVisible" :close-on-click-modal="false" append-to-body>
			<el-form :model="form" style="margin:10px 0;padding:0 30px;min-height:330px" size="mini" label-width="80px" :inline="true">
				...
				<el-form-item label="手机号" required><el-input required v-model="form.phone" style="width:210px" clearable placeholder="请输入手机号"></el-input></el-form-item>
				<el-form-item label="邮箱" required><el-input required v-model="form.email" style="width:210px" clearable placeholder="请输入邮箱"></el-input></el-form-item>
				<el-form-item label="负责业务" required>
					<el-select
						v-model="cpxFzyw"
						placeholder="请选择"
						multiple
						:style="{ width: form.rylx == '1013' ? '514px' : '440px' }"
						:disabled="form.rylx == '1013' ? false : isAllCpx"
					>
						<el-option v-for="(yw, index) in cpxData" :label="yw.mc" :value="yw.mc + ',' + yw.label" :key="index"></el-option>
					</el-select>
				</el-form-item>
				<!-- 2018.12.11 学校用户不显示全部-->
				<el-form-item label="" v-if="form.rylx != '1013'"><el-checkbox v-model="isAllCpx">全部</el-checkbox></el-form-item>
				<br />
...
export default {
	...
	methods: {
		...
		handleEditCommit() {
			...
			//   });
			// }else
			// !this.fzywList.length && this.userInfo.unitType == 1
			if (!this.fzywList.length) {
				this.$alert('请选择负责业务', '提示', {
					confirmButtonText: '确定',
					type: 'warning'
				});
				...
				saveRy({
					...
					xmbh: this.xmbh,
					cpxRy: this.fzywList.join(String.fromCharCode(1)), //this.isAllCpx?"":this.fzywList.join(String.fromCharCode(1))
		...
		handleCommit() {
			...
			} else if (!/\w[-\w.+]*@([A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]{2,14}/.test(this.form.email.trim()) || !this.form.email) {
				this.$alert('请输入正确邮箱地址或地址有误', '提示', {
					confirmButtonText: '确定',
					type: 'warning'
				});
			} else if (!this.isAllCpx && this.cpxFzyw.length == 0 && this.form.rylx != '1013') {
				this.$alert('请选择负责业务', '提示', {
					confirmButtonText: '确定',
					type: 'warning'
				});
			} else if (this.cpxFzyw.length == 0 && this.form.rylx == '1013') {
				this.$alert('请选择负责业务', '提示', {
					confirmButtonText: '确定',
					type: 'warning'
				});
					...
					if (this.isJzuser == 1) {
						if (!this.isAllCpx) {
							...
							this.cpxData.forEach((ele, i, arr) => {
								this.fzywList.push(ele.label + String.fromCharCode(2) + ele.mc);
							});
						}
					} else {
