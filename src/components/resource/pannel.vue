	...
	<div class="zczx-pannel">
		...
		<el-collapse-transition>
			<section v-if="catalogue == 'file'">
				<ul class="file-list">
					...
					<li :data-fjbh="fileList.parentwid" data-type="back" @click="handleFile($event,fileList.wid)" class="file-back"
					 v-if="fileList.parentwid != '-1' && JSON.stringify(fileList)!='{}'">
                    ...
                    wmv:file.fjmc.includes('.zip')?zip:FilePng"
										 alt="">
									</span>
									<span>{{file.fjmc}}</span>
								</a>
						...
						<div class="file-evaluate">
							<div flex colcenter v-if="fileIndex == index">
								<span v-if="file.sfwjml == '0'" @click="handlePraise($event,'1',file.fjbh,index,'list')"><span class="appraise-hp"></span>
									({{file.good}})</span>&nbsp;
								<span v-if="file.sfwjml == '0'" @click="handlePraise($event,'0',file.fjbh,index,'list')"><span class="appraise-cp"></span>
									({{file.bad}})</span>
								<span v-if="file.sfwjml == '0'" style="display:flex;justify-content:center"><span class="appraise-download"></span>
									({{file.download}})</span>&#x3000;
		...
		<el-collapse-transition>
			<section v-if="catalogue == 'record'">
				<oprateRecord :jdList="jdList" :fileCounts="fileCounts" @hadnleChangeTab="hadnleChangeTab" @hadnleChange="hadnleChange"
				 :currentPage="currentPage" :pageSize="pageSize" :total="total" @handleCommit="handleCommit"></oprateRecord>
			</section>
		</el-collapse-transition>

		<el-collapse-transition>
			<section v-if="catalogue == 'suggest'">
				<suggest :jdList="suggestList" @hadnleChange="handleChangejy" :currentPage="currentPage" :pageSize="pageSize"
				 :total="totaljy" @handleSucess="handleSucess" @handleAddyj="handleAddyj"></suggest>
			</section>
		</el-collapse-transition>
		...
		<el-collapse-transition>
			<section v-if="catalogue == 'search'">
				...
				<ul class="file-list">
                    ...
                    wmv:file.name.includes('.zip||.rar')?zip:FilePng"
								 alt="">
				...
				<div v-if="searchTotal > 10">
					<el-pagination background @current-change="handleSearchCurrentChange" :current-page="searchCurpage" layout="prev, pager, next"
					 :total="searchTotal">
					</el-pagination>
				</div>
...
<script>
	...
	export default {
		...
		methods: {
			handleSync() {
				this.$get(this.API.refreshFileIndex, {}).then(res => {
			...
			searchFiles() {
				...
				this.$get(this.API.searchFiles, {
					...
					ml: !this.oldFjpath ? '' : this.oldFjpath + "/"
				}).then(res => {
					if (res.state == "success") {
						if (!res.data.datas) {
							this.searchList = [];
