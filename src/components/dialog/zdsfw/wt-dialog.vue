<template>
  <div>
    <el-dialog title="添加问题" width="700px" top="30px" :visible.sync="visible" :append-to-body="true" :close-on-click-modal="false" @close="$emit('update:show', false)" :show="show">
      <div class="dialog-wt">
          <h5>添加问题</h5>
          <el-form ref="form" :model="formData" label-width="90px" size="mini">
            <el-form-item label="问题描述" required>
                <el-autocomplete
                        ...
                        @select="handleSelect"
                      ></el-autocomplete>
              <!-- <el-input v-model="formData.ms" placeholder="请输入问题描述"></el-input> -->
            </el-form-item>
...
export default {
  ...
  watch: {
    show(n, o) {
      ...
      if (!n) {
        this.formData.ms = this.formData.jg = ''
      } else {
        this.currentPage = 1;
        this.pageActiveServiceProblem();
      }
